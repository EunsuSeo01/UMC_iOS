...
class ViewController: UIViewController {
    ...
    lazy var updateLikedButton: ((UIButton) -> ()) = { likedButton in
        // 비어진 하트면 채워진 하트로 이미지 교체.
        if likedButton.imageView?.image == UIImage(systemName: "heart") {
    ...
    @objc func tappedEllipsisButton() {
        ...
        // us의 기사를 가져온다.
        let americaAction = UIAlertAction(title: "America", style: .default, handler: {_ in
            self.articleViewModel.updateCountry(country: "us")
        })
        // china의 기사를 가져온다.
        let chinaAction = UIAlertAction(title: "China", style: .default, handler: {_ in self.articleViewModel.updateCountry(country: "ch")
        })
        // korea의 기사를 가져온다.
        let koreaAction = UIAlertAction(title: "Korea", style: .default, handler: {_ in self.articleViewModel.updateCountry(country: "kr")
        })
...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        print("DEBUG: ", articleViewModel.getTotalArticlesNumber())
        print(articleViewModel.getTotalFavoriteArticlesNumber())
        return 3
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.displayArticle(article: articleViewModel.getArticle(index: indexPath.row))
